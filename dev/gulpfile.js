...
function handlebars_main() { //в этом таске компилятся страницы, которые были измененые (исп-я ф-я changed)
	// return src('src/layouts/*.html')
		...
		// .pipe(dest('dist/'))
		// .pipe(connect.reload());
return src('src/layouts/**/*.html', { base: 'src/layouts' }) // сохраняем структуру
...
async function asyncAwaitTask() {
	...
	watch('src/*.html', html);
	watch('src/layouts/*.html', handlebars_main); // добавленный таск
	watch('src/layouts/pages/**/*.html', handlebars_main);
